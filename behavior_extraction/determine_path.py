...
ignore_list = ['<java.lang.RuntimeException: void <init>(java.lang.String)>', '<java.lang.Object: void <init>()>','<java.lang.Exception: void <init>()>','<java.lang.Throwable: void <init>()>']
...
def contains_important_apis(args, android_path_list, app_semantics, graph):
    ...
    for i, path in enumerate(android_path_list):
        ...
        for j, node in enumerate(path):
            ...
            if node in ignore_list:
                continue
            print(node)
            if not graph.is_android_lib(node):
                continue
            suspicious = False
            if semantics['xml'] == []:
                tag = 'app'
...
def determine_path(args):
    ...
    contains_important_apis(args, path_list, app_semantics, graph)
    return True, dot_path

def main_single(apps, result_dir):
    ...
    for i, app in enumerate(apps):
        if '1311833082_net.lepeng.batterydoctor' not in app:
            continue
