...
def handle_layout_text(app_name, id, layout, path_decoded, search_range):
    ...
    # return handle_layout_text_from_xml(img_name, path_layout, path_values, search_range, search_widget)
    return handle_layout_text_from_xml(id, path_layout, path_values, search_range)


# def handle_layout_text_from_xml(img_name, path_layout, path_string_values, search_range, search_widget):
def handle_layout_text_from_xml(id, path_layout, path_string_values, search_range):
    ...
    if path_layout is not None:
        ...
        # layout_texts = get_text_from_layout(img_name, layout_content, string_dict, search_range, search_widget)
        layout_texts = get_text_from_layout(id, layout_content, string_dict, search_range)
...
# def get_text_from_layout(img_name, layout_content, string_dict, search_range, search_widget):
def get_text_from_layout(id, layout_content, string_dict, search_range):
    ...
    if id != '':
        parents = get_parent_nodes(root, id) if search_range == 'parent' else [root]
    else:
        parents = [root]
    try:
        id_attrib = '{{{}}}id'.format(ns['android'])
        text_attrib = '{{{}}}text'.format(ns['android'])
        vis_attrib = '{{{}}}visibility'.format(ns['android'])
        src_attrib = '{{{}}}src'.format(ns['android'])
    ...
    for parent in parents:
        for node in parent.iter():
            ...
            # print(node.tag)
            # # print(node.text)
            # if search_widget == True and node.tag != 'TextView':
            #     continue
            # import sys
            # sys.exit()
            # extract texts
            # try:
                # if text_attrib not in node.attrib.keys() and src_attrib in node.attrib.keys() and "drawable" in node.attrib[src_attrib]:
                #     continue
            text_name = ''
            text_content = ''
            if text_attrib in node.attrib.keys():
                text = node.attrib[text_attrib]
                if text.startswith('@string'):
                # map identifier to real contents (described in strings.xml)
                    # print("text: ",text)
                    text_name  = text.split('/', 1)[1]

                    if text.split('/', 1)[1] in string_dict.keys():
                        text_content = string_dict[text.split('/', 1)[1]]
                        if text_content == None:
                            text_content = ''
                    # else:
                    #     text = text.split('/', 1)[1]
                    # if text is not None:
                    #     text = translate_any_to_english(text, True)
                else:
                    text_content = text
            ids = ''
            if id_attrib in node.attrib.keys():
                ids = node.attrib[id_attrib].split('/')[1]
                # if ids is not None:
                #     ids = translate_any_to_english(ids, True)
            
            # print("id: ",id, " text: ",text)
            if ids !='' or text_name != '' or text_content != '':
            # if text is not None or id is not None:
            #     if text is None and src is not None and "drawable" in src:
            #         continue
            #     if text is not None:
            #         print("here")
            #         # r = translate_any_to_english(text, True)
            #     else:
            #         text = ''
            #     if id is not None:
            #         id = id.split("/")[-1]
            #     else:
            #         id = ''
                texts.append([ids,text_name,text_content])
            # except KeyError:
            #     continue
...
def get_resource_path(path_res, prefix, content_name):
    ...
    for name in os.listdir(path_res):
        if content_name == 'strings.xml': # add by x
            path = os.path.join(path_res, prefix, content_name)
