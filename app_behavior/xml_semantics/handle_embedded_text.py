...
def get_largest_image(path_app, image_locations):
    ...
    for loc in image_locations:
        ...
        resize_image(os.path.split(fd_loc)[0], fd_loc)
    ...
    if target_img is not None:
        ...
        # try:
            ...
            #     tmp_img = cv2.imread(fd_loc, cv2.IMREAD_UNCHANGED)
            #     # image_have_alpha_convert_bgr1 = cv2.cvtColor(tmp_img, cv2.COLOR_RGBA2BGR)
            #     # new_path = os.path.join(path_app, 'res/drawable/a.png')
            #     # cv2.imwrite(new_path, image_have_alpha_convert_bgr1)
            #     image_have_alpha_convert_bgr2 = rgba2rgb(tmp_img)
            #     cv2.imwrite(fd_loc, image_have_alpha_convert_bgr2)
            #     target_img = get_image(fd_loc)
        # except Exception:
        #     pass
