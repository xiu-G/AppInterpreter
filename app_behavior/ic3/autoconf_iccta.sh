...
mysql -u $USERNAME -p$PASSWORD $DATABASE_NAME < res/schema
...
sed -i "s/<username>.*<\/username>/<username>$USERNAME<\/username>/" res/jdbc.xml
...
sed -i "s/<password>.*<\/password>/<password>$PASSWORD<\/password>/" res/jdbc.xml
check_return $? "Something went wrong with sed (res/jdbc.xml)."
# sed -i "s/<name>.*<\/name>/<name>$DATABASE_NAME<\/name>/" res/jdbc.xml &> /dev/null
sed -i "s/<name>.*<\/name>/<name>$DATABASE_NAME<\/name>/" res/jdbc.xml
...
sed -i "s/<username>.*<\/username>/<username>$USERNAME<\/username>/" release/res/jdbc.xml
...
sed -i "s/<password>.*<\/password>/<password>$PASSWORD<\/password>/" release/res/jdbc.xml
check_return $? "Something went wrong with sed (release/res/jdbc.xml)."
sed -i "s/<name>.*<\/name>/<name>$DATABASE_NAME<\/name>/" release/res/jdbc.xml &> /dev/null
sed -i "s/<name>.*<\/name>/<name>$DATABASE_NAME<\/name>/" release/res/jdbc.xml
...
sed -i "s/android_jars=.*/android_jars=${PLATFORMS////\\/}/" release/res/iccta.properties
