...
class DefaultXMLParser extends AbstractXMLParser {
		...
		try {
			...
			for (int i = 0; i < nodes.getLength(); ++i) {
				try {
					...
					Logger.verb("ERROR", "Nullpointer Exception in readManifest maybe caused by " + "customized namespace");
					continue;
				}
			}
	...
	public void feedIdIntoGeneralMap(String type, String name, Integer value, boolean isSys) {
		...
		if (Configs.verbose) {
			if (workingMap.containsKey(name) || invWorkingMap.containsKey(value)) {
				System.out.println("[VERB] DefaultXML Parser feedIdIntoGeneralMap name value conflicts at " + name + ":" + value);
			}
		}
	...
	private void resolveIncludes(String resRoot, HashMap<Integer, String> nameMap, HashMap<Integer, AndroidView> viewMap, boolean isSys) {
		...
		while (!work.isEmpty()) {
			...
			for (int i = 0; i < view.getNumberOfChildren(); i++) {
				...
				} else if (sysRGeneralIdMap.get("layout").containsKey(layoutId) && sysId2View.containsKey(sysRGeneralIdMap.get("layout").get(layoutId))) {
	...
	private void resolveIncludesApplicationProject(HashMap<Integer, String> nameMap, HashMap<Integer, AndroidView> viewMap) {
	...
	private void readLayout(String resRoot, HashMap<Integer, String> in, HashMap<Integer, AndroidView> out, boolean isSys) {
		if (debug) {
	...
	private void readLayoutApplicationProject(HashMap<Integer, String> in, HashMap<Integer, AndroidView> out) {
		...
		for (Map.Entry<Integer, String> entry : in.entrySet()) {
			...
			if (file == null) {
				if (Configs.verbose) {
					System.err.println("[WARNING] Cannot find " + layoutFileName + ".xml in " + Configs.resourceLocation);
				}
				continue;
			}
	...
	private void readLayout(String file, AndroidView root, boolean isSys) {
		...
		while (!work.isEmpty()) {
			...
			if (attrMap == null) {
				System.out.println(file + "!!!" + node.getClass() + "!!!" + node.toString() + "!!!" + node.getTextContent());
			}
	...
	private static String getLayoutFilePath(String resRoot, String layoutId, boolean isSys) {
		...
		if ("status_bar_latest_event_ticker_large_icon".equals(layoutId) || "status_bar_latest_event_ticker".equals(layoutId) || "keyguard_screen_status_land".equals(layoutId) || "keyguard_screen_status_port".equals(layoutId)) {
			assert isSys;
	...
	private void readMenu(String resRoot, HashMap<Integer, String> map, HashMap<Integer, AndroidView> viewMap, boolean isSys) {
	...
	private String getMenuFilePath(String project, String menuId, boolean isSys) {
		...
		for (String proj : projectDirs) {
			...
			/*
			 * String file = proj + "/res/menu/" + menuId + ".xml"; if (!new File(file).exists()) { file = null;
			 * }
			 */
			if (file != null) {
				return file;
	...
	private void readStrings(String file, HashMap<Integer, String> idAndStrings, HashMap<String, String> stringFieldAndStrings, HashMap<String, Integer> stringFieldAndIds) {
		Document doc;
		try {
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
	...
	/*
	 * Usually the file name is strings.xml, but it technically can be anything. For now, let's read
	 * strings.xml and strings-*.xml.
	 */
	...
	private void printMap(Map theMap, String name) {
		...
		for (Object key : keys) {
			...
			System.out.println(String.format("KEY Type: %s, %s VALUE Type: %s, %s", key.getClass().getSimpleName(), key.toString(), value.getClass().getSimpleName(), value.toString()));
