...
/**
 * Fixed-point computation to resolve the mutual-dependence between NOpNode nodes. First, it
 * performs simple graph reachability. Then, it does the actual fixed-point computation. Right now,
 * it is iterative but seems to be pretty efficient.
 */
public class FixpointSolver {
	...
	/**
	 * Look at all layout IDs and see what operation nodes use them. Both application and framework
	 * layout IDs are looked at. However, operation nodes are only created for application code. The
	 * result is saved in the map reachingLayoutIds.
	 */
			...
			for (NNode target : graphUtil.reachableNodes(layoutIdNode)) {
				...
				if (opNode.consumesLayoutId()) {
					...
					if (Configs.sanityCheck) {
						...
						Logger.verb("WARNING", layoutIdNode + " reaching " + opNode + " in layoutIdReachability" + " method");
					}
				}
			}
		}
			...
			for (NNode target : graphUtil.reachableNodes(menuIdNode)) {
				...
				if (opNode.consumesMenuId()) {
					...
					if (Configs.sanityCheck) {
						...
						Logger.verb("WARNING", menuIdNode + " reaching " + opNode + " in menuIdReachability" + " method");
					}
				}
			}
		}
	...
	void windowReachability() {
		for (NWindowNode windowNode : NWindowNode.windowNodes) {
			...
			for (NNode target : reachables) {
				...
				NOpNode opNode = (NOpNode) target;
				if ((opNode instanceof NInflate2OpNode || opNode instanceof NAddView1OpNode || opNode instanceof NFindView2OpNode) && reachables.contains(opNode.getReceiver())) {
	...
	void optionsMenuReachability() {
		for (NOptionsMenuNode optionsMenu : flowgraph.activityClassToOptionsMenu.values()) {
			for (NNode target : graphUtil.reachableNodes(optionsMenu)) {
				...
				NOpNode opNode = (NOpNode) target;
				if (opNode instanceof NFindView1OpNode || opNode instanceof NFindView3OpNode || opNode instanceof NAddView2OpNode || opNode instanceof NMenuInflateOpNode) {
					...
					Logger.verb("WARNING", String.format("OptionsMenu Reached %s at %s at %s at line %d", opNode.toString(), fullClassName, methodName, lineNumber));
					continue;
				}
			}
		}
	...
	void viewIdReachability() {
		for (NWidgetIdNode viewIdNode : flowgraph.allNWidgetIdNodes.values()) {
			for (NNode target : graphUtil.reachableNodes(viewIdNode)) {
				...
				NOpNode opNode = (NOpNode) target;
				if (opNode instanceof NFindView1OpNode || opNode instanceof NFindView2OpNode || opNode instanceof NSetIdOpNode) {
	...
	void viewAndListenerAsParameterAndReceiverReachability() {
		...
		for (NNode n : flowgraph.allNNodes) {
			if (n instanceof NViewAllocNode || n instanceof NInflNode || n instanceof NOptionsMenuNode || n instanceof NContextMenuNode) {
				parameterAndReceiverViewReachability(n);
			} else if (n instanceof NOpNode) {
				NOpNode opNode = (NOpNode) n;
				if (opNode instanceof NFindView1OpNode || opNode instanceof NFindView2OpNode || opNode instanceof NFindView3OpNode || opNode instanceof NInflate1OpNode) {
					parameterAndReceiverViewReachability(n);
				}
			}
	...
	void parameterAndReceiverViewReachability(NNode source) {
		...
		for (NNode target : reachables) {
			...
			NOpNode opNode = (NOpNode) target;
			// View as parameter
			if (opNode instanceof NAddView1OpNode || (opNode instanceof NAddView2OpNode && reachables.contains(opNode.getParameter()))) {
			...
			// View as receiver
			if (opNode instanceof NFindView1OpNode || opNode instanceof NFindView3OpNode || opNode instanceof NSetIdOpNode || opNode instanceof NSetTextOpNode || opNode instanceof NSetImageResourceOpNode || (opNode instanceof NSetListenerOpNode && reachables.contains(opNode.getReceiver())) || (opNode instanceof NAddView2OpNode && reachables.contains(opNode.getReceiver()))) {
			...
			for (NLayoutIdNode layoutIdNode : entry.getValue()) {
				if (opNode instanceof NInflate1OpNode) {
					...
					if (windows == null || windows.isEmpty()) {
						if (Configs.verbose) {
							System.out.println("[WARNING] No window reaching " + opNode + ", layoutId: " + layoutIdNode);
						}
						continue;
					}
	...
	void inflationEffectsToParameters(NOpNode opNode, NInflNode inflNode, Map<NOpNode, Set<NOpNode>> reached, Map<NOpNode, Set<NNode>> solution) {
	 ...
	 *
	 * 1) Connect the node to <this> of constructor. Note that View.<init>(Context) is only called from
	 * code, not from inflation. However, let's pretend all constructors could be called to simplify
	 * life. This is fine because we only consider application code.
	 *
	 * 2)
	 */
	...
	void inflNodeToReceiverAndParameter(NNode inflNode) {
		...
		for (NNode n : reachables) {
			...
			// View as receiver
			if (opNode instanceof NFindView1OpNode || opNode instanceof NFindView3OpNode || opNode instanceof NSetIdOpNode || (opNode instanceof NSetListenerOpNode && reachables.contains(opNode.getReceiver())) || (opNode instanceof NAddView2OpNode && reachables.contains(opNode.getReceiver()))) {
	...
	NInflNode doLayoutInflate(Integer layoutId, NNode rootparent) {
		...
		while (!worklist.isEmpty()) {
			...
			if (v.getSootClass() == null) {
				Logger.verb("DEBUG", "AndroidView " + v.getText() + " " + v.getId() + " " + v.getOrigin() + " " + "SootClass is" + " null");
				continue;
			}
	...
	void propagateViewToParameter(Set<NNode> solution, NOpNode node, Map<NOpNode, Set<NOpNode>> reachedMap, Map<NOpNode, Set<NNode>> solutionMap) {
