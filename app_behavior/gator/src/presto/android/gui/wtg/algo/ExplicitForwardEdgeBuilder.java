...
public class ExplicitForwardEdgeBuilder implements Algorithm {
	...
	/**
	 * build gui hierarchy and necessary wtg nodes before creating edges in this stage, no edges are
	 * added to wtg
	 */
	...
	private void buildExplicitForwardEdges(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NObjectNode, HandlerBean> viewToHandlers) {
		...
		for (NObjectNode window : guiHierarchy.keySet()) {
			...
			WTGEdge backEdge = helper.createEdge(wtg, current, current, window, EventType.implicit_back_event, Sets.<SootMethod>newHashSet(), RootTag.implicit_back, Lists.<StackOperation>newArrayList(), Lists.<EventHandler>newArrayList());
			addEdge(newEdges, backEdge);
		}
	}

	private Map<CFGAnalyzerInput, CFGAnalyzerOutput> analyzeCallback(final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NObjectNode, HandlerBean> viewToHandlers) {
		/****************************************************************************/
		/****************************************************************************/
		/***************************** analyze callbacks *****************************/
		/****************************************************************************/
	...
	private void buildWTGEdges(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NMenuNode, NWindowNode> menuToWindows, final Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
		...
		WTGEdge newEdge = wtgHelper.createEdge(wtg, launcher, mainNode, NLauncherNode.LAUNCHER, EventType.implicit_launch_event, Sets.<SootMethod>newHashSet(), RootTag.implicit_launch, stackOps, callbacks);
		addEdge(newEdges, newEdge);


		// build other forward edges
		Multimap<NObjectNode, HandlerBean> widgetToHandlers = buildWidgetHandlers(guiHierarchy, menuToWindows, windowToContextMenus);
	...
	private void buildWTGNodes(final WTG wtg, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NMenuNode, NWindowNode> menuToWindows, final Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
		// build GUI hierarchy
		initializeGUIHierarchy(wtg, guiHierarchy, menuToWindows, windowToContextMenus);
	}

	private Multimap<NObjectNode, HandlerBean> buildWidgetHandlers(Multimap<NObjectNode, NObjectNode> guiHierarchy, Multimap<NMenuNode, NWindowNode> menuToWindows, Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
		...
		for (NObjectNode window : guiHierarchy.keySet()) {
			...
			for (NObjectNode v : underneathViews) {
				...
				for (EventType evt : evtToCallbacks.keySet()) {
					...
					for (SootMethod callback : callbacks) {
						...
						if (!hier.isSibling(declaringClz, windowClz, wtgUtil.activityClass) && !hier.isSibling(declaringClz, windowClz, wtgUtil.dialogClass) && !(hier.isSubclassOf(declaringClz, windowClz) && declaringClz != windowClz)) {
			...
			if (window instanceof NWindowNode) {
				Collection<Pair<NObjectNode, NContextMenuNode>> viewToContextMenus = windowToContextMenus.get((NWindowNode) window);
				for (Pair<NObjectNode, NContextMenuNode> viewToContextMenu : viewToContextMenus) {
					NObjectNode v = viewToContextMenu.getO1();
					{
						SootMethod callback = wtgHelper.getCallback(window.getClassType(), MethodNames.onCreateContextMenuSubSig);
						if (callback != null) {
							addHandlerBean(viewToHandlers, window, v, EventType.long_click, callback, true);
						}
					}
					{
						SootMethod callback = wtgHelper.getCallback(v.getClassType(), MethodNames.viewOnCreateContextMenuSubSig);
						if (callback != null) {
							addHandlerBean(viewToHandlers, window, v, EventType.long_click, callback, true);
						}
					}
			...
			if (window instanceof NActivityNode) {
				...
				if (om != null) {
					SootMethod callback = wtgHelper.getCallback(window.getClassType(), MethodNames.onCreateOptionsMenuSubsig);
					if (callback != null) {
				...
				if (window instanceof NContextMenuNode) {
					handlers = Lists.newArrayList(new String[] {MethodNames.onContextItemSelectedSubSig, MethodNames.onMenuItemSelectedSubSig});
				} else {
					handlers = Lists.newArrayList(new String[] {MethodNames.onOptionsItemSelectedSubSig, MethodNames.onMenuItemSelectedSubSig});
				}
		...
		if (Configs.asyncStrategy == AsyncOpStrategy.Default_Special_Async || Configs.asyncStrategy == AsyncOpStrategy.All_Special_Async) {
	...
	private void initializeGUIHierarchy(final WTG wtg, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NMenuNode, NWindowNode> menuToWindows, final Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
		...
		while (!windowNodeList.isEmpty()) {
			...
			if (windowNode instanceof NActivityNode) {
				...
				Logger.err(getClass().getSimpleName(), "the current definition of window is of type: activity," + " dialog and menu, but it is " + windowNode);
			}
	...
	private void initWindow(NContextMenuNode menu, NWindowNode owner, NObjectNode view, Multimap<NMenuNode, NWindowNode> menuToWindows, Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
		initWindow(menu, owner, menuToWindows);
		windowToContextMenus.put(owner, new Pair<NObjectNode, NContextMenuNode>(view, menu));
	}

	private void generateActivityImplicitHandlers(NActivityNode activity, Multimap<NObjectNode, HandlerBean> viewToHandlers) {
		SootClass activityClass = activity.c;
		{
			...
			if (handler != null) {
        // EventType.implicit_on_activity_newIntent, handler);
        // viewToHandlers.put(activityNode, callback);
				addHandlerBean(viewToHandlers, activity, activity, EventType.implicit_on_activity_newIntent, handler, true);
			}
		}
        ...
        // viewToHandlers.put(dialogNode, callback);
				addHandlerBean(viewToHandlers, activity, activity, EventType.implicit_on_activity_result, handler, true);
			}
		}
		{
			// handle the rest of explicit handlers on activity node
			List<String> keyActions = Lists.newArrayList("boolean onKeyDown(int,android.view.KeyEvent)", "boolean onKeyUp(int,android.view.KeyEvent)");
	...
	private void generateDialogImplicitLifeCycleHandlers(NDialogNode dialogNode, Multimap<NObjectNode, HandlerBean> viewToHandlers) {
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.dialog_dismiss,
        // handler);
        // viewToHandlers.put(dialogNode, callback);
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.dialog_press_key,
        // handler);
        // viewToHandlers.put(dialogNode, callback);
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode,
        // EventType.implicit_lifecycle_event, handler);
        // viewToHandlers.put(dialogNode, callback);
	...
	private void buildActivityForwardEdges(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final NObjectNode window, final Map<CFGAnalyzerInput, CFGAnalyzerOutput> analyzeOutput, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NObjectNode, HandlerBean> viewToHandlers) {
		...
		for (NObjectNode view : allViews) {
			for (HandlerBean bean : viewToHandlers.get(view)) {
				...
				if (avoid) {
					...
					WTGEdge newEdge = helper.createEdge(wtg, srcNode, srcNode, guiWidget, event, handlers, RootTag.cyclic_edge, stackOps, callbacks);
					addEdge(newEdges, newEdge);
				}
			}
		}
	...
	private void buildActivityForwardEdge(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final NObjectNode sourceWindow, final NObjectNode targetWindow, final EventType event, final NObjectNode guiWidget, final Set<SootMethod> eventHandlers) {
		...
		if (targetWindow instanceof NActivityNode) {
			...
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, eventHandlers, RootTag.start_activity, stackOps, callbacks);
			addEdge(newEdges, newEdge);
		} else if (targetWindow instanceof NDialogNode) {
			/* if the target window is dialog */
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, eventHandlers, RootTag.show_dialog, stackOps, callbacks);
			addEdge(newEdges, newEdge);
		} else if (targetWindow instanceof NContextMenuNode) {
			/* if the target window is context menu */
			/*
			 * WTGEdge newEdge = null; if (!subsig.equals(MethodNames.onCreateContextMenuSubSig)) { newEdge =
			 * helper.createEdge(wtg, source, target, guiWidget, event, eventHandler, RootTag.open_context_menu,
			 * stackOps, callbacks); } else { newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
			 * null, RootTag.open_context_menu, stackOps, callbacks); }
			 */
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, eventHandlers, RootTag.open_context_menu, stackOps, callbacks);
			addEdge(newEdges, newEdge);
			...
			/*
			 * WTGEdge newEdge = null; if (!subsig.equals(MethodNames.onCreateOptionsMenuSubsig)) { newEdge =
			 * helper.createEdge(wtg, source, target, guiWidget, event, eventHandler, RootTag.open_options_menu,
			 * stackOps, callbacks); } else { newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
			 * null, RootTag.open_options_menu, stackOps, callbacks); }
			 */
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, eventHandlers, RootTag.open_options_menu, stackOps, callbacks);
			addEdge(newEdges, newEdge);
		} else {
	...
	private void buildDialogOrMenuForwardEdges(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final NObjectNode window, final Map<CFGAnalyzerInput, CFGAnalyzerOutput> analyzeOutput, final Multimap<NObjectNode, NObjectNode> guiHierarchy, final Multimap<NObjectNode, HandlerBean> viewToHandlers) {
	...
	private void buildDialogOrMenuForwardEdge(final Multimap<WTGEdgeSig, WTGEdge> newEdges, final WTG wtg, final NObjectNode sourceWindow, final NObjectNode targetWindow, final HandlerBean callback) {
		...
		if (targetWindow instanceof NActivityNode) {
			/* if the target window is activity */
			// forward edge
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, handlers, RootTag.start_activity, stackOps, callbacks);
			addEdge(newEdges, newEdge);
		} else if (targetWindow instanceof NDialogNode) {
			/* if the target window is dialog */
			WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event, handlers, RootTag.show_dialog, stackOps, callbacks);
			addEdge(newEdges, newEdge);
	...
	private void addHandlerBean(Multimap<NObjectNode, HandlerBean> beans, NObjectNode window, NObjectNode widget, EventType evt, SootMethod callback, boolean isWidgetKey) {
		NObjectNode key = widget;
		if (!isWidgetKey) {
			key = window;
		}
		for (HandlerBean bean : beans.get(key)) {
			if (evt != EventType.implicit_async_event && bean.getEvent() != EventType.implicit_async_event && bean.getWindow() == window && bean.getGUIWidget() == widget && bean.getEvent() == evt) {
				bean.addHandler(callback);
				return;
			} else if (evt == EventType.implicit_async_event && bean.getEvent() == EventType.implicit_async_event && bean.getWindow() == window && bean.getGUIWidget() == widget && bean.getHandlers().size() == 1 && bean.getHandlers().contains(callback)) {
				return;
			}
		}
	...
	private void handleViewAndActivityAsyncOps(Multimap<NObjectNode, NObjectNode> guiHierarchy, Multimap<NObjectNode, HandlerBean> viewToHandlers) {
			...
			for (NObjectNode widget : widgets) {
				...
				for (NObjectNode window : windows) {
					for (SootMethod callback : runnableCallbacks) {
						addHandlerBean(viewToHandlers, window, (NObjectNode) widget, EventType.implicit_async_event, callback, true);
					}
				}
			}
		}
	...
	private void handleHandlerAsyncOps(Multimap<NMenuNode, NWindowNode> menuToWindows, Multimap<NObjectNode, HandlerBean> viewToHandlers) {
		...
		for (NNode nnode : guiOutput.getFlowgraph().allNNodes) {
			if (nnode instanceof NActivityNode) {
				...
				for (SootMethod callback : helper.getCallbacks(actCls, MethodNames.onActivityCreateSubSig, MethodNames.onActivityStartSubSig, MethodNames.onActivityRestartSubSig, MethodNames.onActivityResumeSubSig, MethodNames.onActivityPauseSubSig, MethodNames.onActivityStopSubSig, MethodNames.onActivityDestroySubSig, MethodNames.activityOnNewIntentSubSig)) {
				...
				for (SootMethod callback : helper.getCallbacks(diaCls, MethodNames.onDialogCreateSubSig, MethodNames.onDialogStartSubSig, MethodNames.onDialogStopSubSig)) {
				...
				for (NWindowNode owner : menuToWindows.get(omNode)) {
					...
					for (SootMethod callback : helper.getCallbacks(ownerCls, MethodNames.onCreateOptionsMenuSubsig, MethodNames.onPrepareOptionsMenuSubsig, MethodNames.onCloseOptionsMenuSubsig)) {
				...
				for (NWindowNode owner : menuToWindows.get(ctxNode)) {
					...
					for (SootMethod callback : helper.getCallbacks(ownerCls, MethodNames.onCreateContextMenuSubSig, MethodNames.onCloseContextMenuSubsig)) {
			...
			for (NNode nnode : queryHelper.allVariableValues(rcvNode)) {
				if (nnode instanceof NAllocNode) {
					for (NObjectNode sourceWindow : handlerToSourceWindows.get((NAllocNode) nnode)) {
						for (SootMethod callback : runnableCallbacks) {
							addHandlerBean(viewToHandlers, sourceWindow, sourceWindow, EventType.implicit_async_event, callback, true);
						}
					}
				}
			}
