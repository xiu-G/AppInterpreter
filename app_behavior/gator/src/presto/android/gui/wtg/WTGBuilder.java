...
public class WTGBuilder {
	...
	private void building() {
		...
		ExplicitForwardEdgeBuilder explicitForwardEdgeBuilder = new ExplicitForwardEdgeBuilder(guiOutput, flowgraphRebuilder);
		...
		Multimap<WTGEdgeSig, WTGEdge> stage2 = new LifecycleForwardEdgeBuilder(guiOutput, flowgraphRebuilder).buildEdges(wtg, stage1, ownership);
		Logger.verb(getClass().getSimpleName(), "stage 2 finishes");
		Multimap<WTGEdgeSig, WTGEdge> stage3 = new CloseWindowEdgeBuilder(guiOutput, flowgraphRebuilder).buildEdges(wtg, stage2, ownership);
		Logger.verb(getClass().getSimpleName(), "stage 3 finishes");
		Multimap<WTGEdgeSig, WTGEdge> stage4 = new CallbackSequenceBuilder(guiOutput, flowgraphRebuilder).buildEdges(wtg, stage3, ownership);
		Logger.verb(getClass().getSimpleName(), "stage 4 finishes");
		Multimap<WTGEdgeSig, WTGEdge> stage5 = new BackEdgeBuilder(guiOutput, flowgraphRebuilder).buildEdges(wtg, stage4, ownership);
		Logger.verb(getClass().getSimpleName(), "stage 5 finishes");
		Multimap<WTGEdgeSig, WTGEdge> stage6 = new LifecycleCloseEdgeBuilder(guiOutput, flowgraphRebuilder).buildEdges(wtg, stage5, ownership);
	...
	private void collectStatistic(WTGBuilder wtgBuilder, WTGAnalysisOutput wtgOutput) {
		...
		Logger.verb(getClass().getSimpleName(), "\\texttt{" + Configs.benchmarkName + "}\t& nodes: " + numOfNodes + "\t& stage2: " + stage2.size() + "\t& stage4: " + stage4.size() + "(" + diffPartialEdge(stage2, stage4) + ")" + "\t& stage6: " + stage6.size() + "(" + diffFullEdge(stage4, stage6) + ")" + "\t& " + Configs.sDepth + "\t& " + execTime);
	}
	...
	// diff the signatures of two sets of edges, by comparing only part of their attributes, e.g.,
	// source,
	// target and event handling callbacks. The reason it is needed because edges generated from stage 1
	// (defined in the paper) do not contain push/pop labels, but edges generated in stage 2 do have
	// such
	...
	private String diffPartialEdge(Multimap<WTGEdgeSig, WTGEdge> first, Multimap<WTGEdgeSig, WTGEdge> second) {
		int adds = 0;
		{
			...
			for (WTGEdgeSig oldSig : first.keySet()) {
				...
				for (WTGEdgeSig newSig : second.keySet()) {
					if (oldSig.getSourceNode() == newSig.getSourceNode() && oldSig.getTargetNode() == newSig.getTargetNode() && oldSig.getWTGHandlers().equals(newSig.getWTGHandlers()) && !newSigs.contains(newSig)) {
						match = true;
						newSigs.add(newSig);
						break;
					}
		...
		}
		{
			...
			for (WTGEdgeSig newSig : second.keySet()) {
				boolean match = false;
				for (WTGEdgeSig oldSig : first.keySet()) {
					if (oldSig.getSourceNode() == newSig.getSourceNode() && oldSig.getTargetNode() == newSig.getTargetNode() && oldSig.getWTGHandlers().equals(newSig.getWTGHandlers()) && !oldSigs.contains(oldSig)) {
						match = true;
						oldSigs.add(oldSig);
						break;
					}
